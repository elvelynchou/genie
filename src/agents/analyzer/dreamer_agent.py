...
class DreamerAgent(BaseAgent):
    ...
    async def run(self, params: DreamerInput, chat_id: str) -> AgentResult:
        ...
        fact_texts = "\n".join([f"- [{f.get('entities', 'unknown')}] {f.get('content', '')}" for f in facts])
        
        # 2. Ask Gemini to find patterns and synthesize
        consolidation_prompt = f"""
        你是一个顶尖的系统架构师和投资研究主管。
        你正在进行“离线梦境”阶段：复盘并合并过去积累的碎片化事实，将其升华为长效策略。

        【待处理的事实碎片】：
        {fact_texts[:15000]}

        【任务】：
        1. 寻找关联：哪些事实指向了同一个宏观趋势？（例如：多个关于铁矿石的新闻指向了中东基建潮）。
        2. 提取策略：生成 1-2 条新的“宏观策略节点 (Strategy, depth=0)”。
        3. 优化逻辑：生成 1 条“执行逻辑节点 (Logic, depth=1)”，总结最佳的操作路径。

        要求输出严格的 JSON 格式：
        {{
          "new_strategies": [
            {{"insight": "策略描述", "entities": ["关键词"]}}
          ],
          "new_logics": [
            {{"insight": "逻辑描述", "entities": ["关键词"]}}
          ]
        }}
        ...
        try:
            ...
            for s in dream_data.get("new_strategies", []):
                ...
                if vector:
                    await self.redis_mgr.store_vector(f"strat_dream_{timestamp}", vector, f"[Synthesized Strategy] {content}", entities, depth=0)
            ...
            for l in dream_data.get("new_logics", []):
                ...
                vector = await loop.run_in_executor(None, lambda: self.orchestrator.get_embedding(content))
                if vector:
                    await self.redis_mgr.store_vector(f"logic_dream_{timestamp}", vector, f"[Synthesized Logic] {content}", entities, depth=1)
